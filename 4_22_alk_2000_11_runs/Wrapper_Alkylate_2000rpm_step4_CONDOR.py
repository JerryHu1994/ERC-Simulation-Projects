...
ivc_range_arr = np.array([360, 370,	380, 385, 390, 395, 400, 405, 410, 415, 420, 425, 430])
Wiebe_theta_0_arr = np.array([1, 4, 9,10,10,10,10,10,10,10,10,10,12])
Wiebe_delta_theta_arr = np.array([40, 40, 45, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]) 
...
tic=time.time()
fname='Alk_2000_step4_CONDOR.xlsx'

wb=load_workbook(fname)
name=datetime.datetime.now().strftime("20%y%m%d_%H-%M.%S_Alk_2000rpm_step4_CONDOR")
ws=wb.active
...
P_ivc=((9.603968345715091e-04)*(2.758853385382365e+02)*(int(n)))/(0.00052138)/1e5 #Recalculate Pressure to maintain Mass
ws['A17']=P_ivc
