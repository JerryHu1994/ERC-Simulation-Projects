...
ivc_range_arr = np.array([340, 350, 355, 360, 365])
Wiebe_theta_0_arr = np.array([3, 11, 16, 16, 16])
Wiebe_delta_theta_arr = np.array([43, 42, 42, 42, 42]) 
BFlag_arr = np.array([0.95, 0.95, 0.95, 0.95, 0.95])
...
tic=time.time()
fname='Alk_1000_step4_CONDOR.xlsx'

wb=load_workbook(fname)
name=datetime.datetime.now().strftime("20%y%m%d_%H-%M.%S_Alk_1000rpm_step4_CONDOR")
ws=wb.active
...
P_ivc=((9.407350110162119e-04)*(275.896)*(int(n)))/(0.00052138)/1e5 #Recalculate Pressure to maintain Mass
ws['A17']=P_ivc
