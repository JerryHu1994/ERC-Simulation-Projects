...
#    gas2 = ct.Solution('TRF_EtOH_PAH.cti')
...
#    print(x_init)
    ...
    keep[gas.species_index('h2o')]=1; keep[gas.species_index('h2')]=0; 
    ...
    outdat=np.zeros((step,14))
    SpecMatrix=np.zeros((step,gas.X.shape[0]))
    #burnflag=0.82  #flag to stop combustion if unburned mass gets too low
#    y_old=unb.thermo.Y
    
    for i in range(step):#step):
#        if (unb.thermo.T < 500) & (Chemflag == 1):
#            unb.chemistry_enabled=False
#        elif (unb.thermo.T > 500) & (Chemflag == 1):
#            unb.chemistry_enabled=True
#            Chemflag=7
#        else:
#            unb.chemistry_enabled=False
        if Chemflag == 1:
